...
exports.uploadCsv = function(req,res)
{
	...
	User.uploadCsv(req.file.path,function(err,user){
		if(err) return res.status(401).send('Data not upload');
		res.send(user);
	})
}
...
{
	User.downloadCsv(function(err,user){
		if(err) return res.status(401).send('Data not upload');
		res.send(user);
	})
}
...
exports.register =function(req,res){
	var data = req.body;
	User.register(data,function(err,userreg){
		if(err) return res.status(401).send('User not registered.');
		res.send(userreg);
	})
}
...
{
	var data = req.body;
	User.login(data,function(err,userlogin){
		if(err) return res.status(401).send('User not registered.');
		res.send(userlogin);
	})
}